...
class InternVL_V1_5(BaseModel):
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        if len(data['embedding']) > 0:
            embeddings = torch.cat(data['embedding'], dim=0)
        else:
            embeddings = None
            print('warning: embeddings is None')
        ...
        input_ids = data['input_ids'] # 过tokenizer后的，参照testInternvl如何做图像token替换
        ### 我们要在底下做替换，想办法把.pt embedding穿进去，对embedding后的替换，但是我们可以提前记录位置，找unused_token_140在哪多长（我们可以提前在数据中prepare好）
        ## emb_mask = data["embedding_mask"]
    ...
    def _llm_forward(
        ...
        embeds_token = '[UNUSED_TOKEN_140]'
        embeds_token_id = self.tokenizer.convert_tokens_to_ids(embeds_token)
        ...
        ## try
        ## except raise error (但是要同时兼容一下这个没有embedding输入的情况)
        if embeddings != None:
            # print('here resampler embedding')
            selected_embed = (input_ids == embeds_token_id)
            try:
                input_embeds[
                    selected_embed] = input_embeds[selected_embed] * 0.0 + embeddings.reshape(
                        -1, C)
            except Exception as e:
                print(f'warning: {e}, input_embeds[selected].shape='
                    f'{input_embeds[selected].shape}, '
                    f'vit_embeds.shape={embeddings.shape}')
                raise NotImplementedError
    
        # 11.1 为了验证试验，注释掉这块插入vit_embeding的代码
        if pixel_values != None:
            # print('here image embedding')
            ...
            selected = (input_ids == self.model.img_context_token_id)  ## 他用的是embed之前，tokenize之后的，所以可以有id编号
            try:
                input_embeds[
